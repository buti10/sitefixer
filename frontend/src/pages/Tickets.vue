...
function humanStatus(raw: string | null | undefined): string {
  ...
  if (!v) return 'Neu'
  if (['neu', 'new', 'open', 'offen', 'pending'].includes(v)) return 'Neu'
  if (['await', 'angebot', 'angebot gesendet', 'waiting', 'wartend'].includes(v)) {
    return 'Angebot gesendet (wartet auf Zahlung)'
  }
  if (['paid', 'bezahlt', 'paid_ok'].includes(v)) return 'Bezahlt'
  if (['closed', 'done', 'erledigt', 'abgeschlossen'].includes(v)) return 'Abgeschlossen'
  if (['cancelled', 'abgebrochen', 'abgelehnt'].includes(v)) return 'Abgebrochen'
...
onMounted(async () => {
  try {
    ...
    rows.value = raw.map((r: any) => ({
      ...
      email: r.email ?? '',
      domain: r.domain ?? '',
      status: r.status ?? null,              // NEU
      handler: r.status_handler ?? null,     // NEU (kommt aus WP-DB)
    }))
  } catch (e: any) {
...
<template>
  <div class="p-6">
    <h1 class="text-2xl font-semibold mb-4">Kunden-Tickets-2</h1>
    ...
    <table v-else class="min-w-full text-sm border">
      <thead class="bg-gray-100">
        <tr>
          ...
          <th class="p-2 text-left">Bearbeiter</th>
          <th class="p-2"></th>
        </tr>
      </thead>
      <tbody>
    ...
    <!-- Detail-Modal bleibt wie gehabt, optional kannst du dort auch status/handler anzeigen -->
    <div v-if="showDetail" class="fixed inset-0 bg-black/30 flex items-center justify-center">
      <div class="bg-white rounded-xl p-6 w-[520px] max-w-full">
        ...
        <div v-else class="space-y-1">
          ...
          <div><b>Status:</b> {{ humanStatus(detail.status) }}</div>
          <div><b>Bearbeiter:</b> {{ detail.status_handler || '—' }}</div>
        ...
        <div class="mt-5 text-right">
          <button class="px-4 py-1 rounded bg-gray-200" @click="showDetail = false">
            Schließen
          </button>
        </div>
      </div>
