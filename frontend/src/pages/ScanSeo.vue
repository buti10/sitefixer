    ...
    <div v-else class="space-y-6">
      ...
      <div class="grid gap-6 lg:grid-cols-[minmax(0,2fr),minmax(0,1.5fr)]">
        ...
        >
          <div class="flex items-start justify-between gap-3">
            ...
            >
              <div
                ...
                :class="scoreBgClass(lastScan?.overall_score)"
              ></div>
          ...
          <div v-if="lastScan" class="mt-4 grid grid-cols-3 gap-2 text-xs">
            <div class="space-y-0.5">
              <div class="text-slate-400">Performance</div>
              <div class="font-semibold">
                {{ lastScan.performance_score ?? '–' }}/100
              </div>
            </div>
            <div class="space-y-0.5">
              <div class="text-slate-400">SEO</div>
              <div class="font-semibold">
                {{ lastScan.seo_score ?? '–' }}/100
              </div>
            </div>
            <div class="space-y-0.5">
              <div class="text-slate-400">Probleme</div>
              <div class="font-semibold">
                <span class="text-red-300">{{ lastScan.critical_count }}</span> /
                <span class="text-amber-300">{{ lastScan.warning_count }}</span>
              </div>
            </div>
          </div>
          ...
          <div v-if="baselineScan" class="mt-4 pt-3 border-t border-white/10 text-xs text-slate-300">
            ...
            · Score:
            <span class="font-medium text-white">
              {{ baselineScan.overall_score ?? '–' }}/100
            </span>
          </div>
        </div>
      </div>
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400">
                  Score:
                  <span class="font-medium">{{ scan.overall_score ?? '–' }}/100</span>
                  · Performance:
                  <span>{{ scan.performance_score ?? '–' }}/100</span>
                  · SEO:
                  <span>{{ scan.seo_score ?? '–' }}/100</span>
                </div>
              </div>
            </div>
...
// Limit-Einstellung (später ggf. aus Backend)
...
// Ticket laden (falls nicht übergeben)
onMounted(async () => {
  if (!ticket.value) {
    try {
