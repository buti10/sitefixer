# app/modules/wp_repair/modules/plugins/scan.py
...
# Plugins enthalten oft PHP/JS – optional auch .json/.css, aber für “Code-Risiken” reichen diese:
_EXTS = (".php", ".phtml", ".php5", ".js")

# gleiche Pattern-Philosophie wie themes/scan.py, aber Kind = plugin_*
_PATTERNS = [
    ...
    ("disable_errors", "low", re.compile(
        r"(error_reporting\s*\(\s*0\s*\)|ini_set\s*\(\s*['\"]display_errors['\"]\s*,\s*['\"]0['\"]\s*\))",
        re.I
    )),
]
...
def plugin_code_scan_apply(
    ...
    *,
    action_id: str,
    plugin_root: str,         # wp-content/plugins/<slug> ODER Datei darin
    max_files: int = 800,
    ...
    try:
        ...
        for path in _iter_files(sftp, plugin_root, max_files=max_files, errors=errors):
            try:
                ...
                log_finding(
                    path=path,
                    severity="low",
                    kind="plugin_read_error",
                    detail={"engine": "plugin_scan", "error": str(e)},
                )
                counts["scanned"] += 1
