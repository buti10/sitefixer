...
# Optional, wenn du Flask Fehler automatisch loggen willst:
#app = Flask(__name__)
#app.logger.addHandler(file_handler)
...
#@app.route('/kunde/<int:ticket_id>/sftp-explorer')

@app.route('/kunde/<int:ticket_id>/sftp-explorer')
def sftp_explorer(ticket_id):
    kunden = get_wp_customers()
    kunde = next((k for k in kunden if k['ticket_id'] == ticket_id), None)
    if not kunde:
        return "Kunde nicht gefunden", 404
...
def all_exception_handler(error):
    ...
    return "Interner Serverfehler (siehe logs/error.log)", 500
